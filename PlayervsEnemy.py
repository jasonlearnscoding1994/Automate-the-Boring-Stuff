...
while True: # Loop is used to deal damage to player and enemy using a randomly generated number.
    if check.y == 1: # Check if correct "run" action has been entered. If so, break loop.
        ...
        print('')
        if newPlayerHealth > 0 and newEnemyHealth <= 0: # Check to see if player health is more than zero and whether enemy health has dropped below zero, if so, display win text and break loop.
            print('YOU WIN.')
            break
